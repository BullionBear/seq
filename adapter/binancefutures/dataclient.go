...
// NewBinanceFuturesDataClient creates a new Binance spot data client
// It initializes both the WebSocket client for real-time data and the HTTP client for REST API requests
func NewBinanceFuturesDataClient(catalog *catalog.Catalog, msgBus *msgbus.MsgBus) *BinanceFuturesDataClient {
	httpClient := NewBinanceFuturesHTTPClient(catalog, msgBus)
...
// pushRateMs selects the push rate: 100ms / 250ms / 500ms (>=1000 → 500ms).
func (c *BinanceFuturesDataClient) SubscribeDepthUpdate(symbolID int, depthLevel int, pushRateMs int) {
	c.subsLock.Lock()
	defer c.subsLock.Unlock()

	pushRate := pushRateFromMs(pushRateMs)
	levels := 0
	switch depthLevel {
	case 5, 10, 20:
		levels = depthLevel
...
func (c *BinanceFuturesDataClient) SubscribeKline(symbolID int, interval string) {
	iv, err := common.ParseInterval(interval)
	if err != nil || iv == common.IntervalUnknown {
		log().Error().Str("interval", interval).Msg("Invalid kline interval for Binance Futures")
		return
	}
...
func (c *BinanceFuturesDataClient) ReqDepthSnapshot(symbolID int, limit int) error {
	return c.httpClient.ReqDepthSnapshot(symbolID, limit)
}

// ReqHistoricalKline requests historical klines via HTTP REST API.
func (c *BinanceFuturesDataClient) ReqHistoricalKline(symbolID int, interval string, startTimeNs, endTimeNs uint64, limit int) error {
...
func (c *BinanceFuturesDataClient) Connect(ctx context.Context) error {
	...
	if len(streams) == 0 {
		log().Warn().Msg("No subscriptions configured, skipping Binance Futures connection")
		return nil
	}

	url := c.buildStreamURL(streams)
	log().Info().Str("url", url).Int("streams", len(streams)).Msg("Connecting to Binance Futures WebSocket")
	...
	log().Info().Msg("Connected to Binance Futures WebSocket")
	return nil
}

// Disconnect closes the WebSocket connection
func (c *BinanceFuturesDataClient) Disconnect() {
	...
	log().Info().Msg("Disconnected from Binance Futures WebSocket")
}

// buildStreamList builds the list of streams to subscribe
func (c *BinanceFuturesDataClient) buildStreamList() []string {
	c.subsLock.RLock()
	defer c.subsLock.RUnlock()
...
func (c *BinanceFuturesDataClient) registerStream(streamName string, symbol *catalog.Symbol) {
...
func (c *BinanceFuturesDataClient) buildStreamURL(streams []string) string {
	if len(streams) == 0 {
		return BaseWsURL + "/ws"
	}
	if len(streams) == 1 {
...
func (c *BinanceFuturesDataClient) subscribeToStream(symbolID int, streamType string) {
	symbol, err := c.catalog.GetSymbol(symbolID)
	if err != nil {
...
func (c *BinanceFuturesDataClient) subscribeToKlineStream(symbolID int, iv common.Interval) {
...
func (c *BinanceFuturesDataClient) subscribeToDepthStream(symbolID int, opts *DepthSubscriptionOptions) {
	symbol, err := c.catalog.GetSymbol(symbolID)
	if err != nil {
		log().Error().Err(err).Int("symbolID", symbolID).Msg("Failed to get symbol for depth subscription")
		return
...
func (c *BinanceFuturesDataClient) sendSubscription(streamName string) {
...
func (c *BinanceFuturesDataClient) pingLoop() {
...
func (c *BinanceFuturesDataClient) handleDisconnect() {
	c.connected.Store(false)
...
func (c *BinanceFuturesDataClient) processMessage(data []byte) {
...
func (c *BinanceFuturesDataClient) processStreamMessage(stream, data []byte) {
	c.streamMapLock.RLock()
...
func (c *BinanceFuturesDataClient) getPrecision(symbolID int) (symbolPrecision, bool) {
	c.streamMapLock.RLock()
...
//	  "asks": [["price","qty"],...]
//	}
func (c *BinanceFuturesDataClient) processDepthSnapshot(symbolID int, data []byte) {
...
// Binance futures depth update format:
//
//	{
//	  "e": "depthUpdate",
...
//	  "a": [["price","qty"],...]   // Asks
//	}
func (c *BinanceFuturesDataClient) processDepthUpdate(symbolID int, data []byte) {
	prec, ok := c.getPrecision(symbolID)
	if !ok {
		return
	}
	...
	// Parse update IDs. Futures provides pu (previous final update ID)
	// for continuity; fall back to U-1 when pu is absent.
	firstUpdateID, _ := jsonparser.GetInt(data, "U")
	finalUpdateID, _ := jsonparser.GetInt(data, "u")
	prevUpdateID, errPu := jsonparser.GetInt(data, "pu")
	if errPu != nil {
		prevUpdateID = firstUpdateID - 1
	}
	depthUpdate.PreviousDepthID = int(prevUpdateID)
...
//	  }
//	}
func (c *BinanceFuturesDataClient) processKline(symbolID int, data []byte) {
...
func (c *BinanceFuturesDataClient) processTrade(symbolID int, data []byte) {
	var tick event.Tick
...
func (h *wsEventHandler) OnOpen(socket *gws.Conn) {
	log().Info().Msg("Binance Futures WebSocket connection opened")
	_ = socket.SetDeadline(time.Now().Add(wsPingInterval + wsPingWait))
}

func (h *wsEventHandler) OnClose(socket *gws.Conn, err error) {
	log().Info().Err(err).Msg("Binance Futures WebSocket connection closed")
}
