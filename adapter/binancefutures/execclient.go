...
// BinanceFuturesExecutionClient handles Binance USD-M futures order execution via WebSocket API
...
type BinanceFuturesExecutionClient struct {
...
// NewBinanceFuturesExecutionClient creates a new Binance futures execution client
func NewBinanceFuturesExecutionClient(catalog *catalog.Catalog, msgBus *msgbus.MsgBus, accountID int, apiKeyName string, walletID int) (*BinanceFuturesExecutionClient, error) {
...
func (c *BinanceFuturesExecutionClient) Connect(ctx context.Context) error {
	...
	log().Info().Int("accountID", c.accountID).Msg("Connected to Binance Futures WebSocket API for trading")
	return nil
}

// Disconnect closes the WebSocket connection
func (c *BinanceFuturesExecutionClient) Disconnect() {
	...
	log().Info().Int("accountID", c.accountID).Msg("Disconnected from Binance Futures WebSocket API")
}

// SubmitOrder submits a new order via WebSocket
func (c *BinanceFuturesExecutionClient) SubmitOrder(clientOrderID int, symbolID int, side common.Side, orderType common.OrderType, timeInForce common.TimeInForce, price float64, quantity float64) error {
	...
	isPostOnly := orderType == common.OrderTypeLimit && timeInForce == common.TimeInForcePO
...
func (c *BinanceFuturesExecutionClient) CancelOrder(symbolID int, orderID int, clientOrderID int) error {
	symbol, err := c.catalog.GetSymbol(symbolID)
	if err != nil {
		return err
	}
...
func (c *BinanceFuturesExecutionClient) CancelAllOrders(symbolID int) error {
	symbol, err := c.catalog.GetSymbol(symbolID)
	if err != nil {
		return err
	}
...
// walletType is accepted for interface compatibility; USD-M futures queries the futures account.
func (c *BinanceFuturesExecutionClient) ReqBalanceSnapshot(walletType common.WalletType) error {
	c.bufLock.Lock()
	defer c.bufLock.Unlock()
	...
	requestID := c.requestID.Add(1)
	msg := c.buildAccountBalanceRequest(timestamp, signature, requestID)
...
func (c *BinanceFuturesExecutionClient) SubscribeOrderUpdate() error {
...
func (c *BinanceFuturesExecutionClient) SubscribeFill() error {
...
func (c *BinanceFuturesExecutionClient) SubscribeBalance() error {
...
func (c *BinanceFuturesExecutionClient) hasAnySubscription() bool {
...
func (c *BinanceFuturesExecutionClient) ensureUserDataStreamSubscribed() error {
...
func (c *BinanceFuturesExecutionClient) buildUserDataStreamSubscribeRequest(timestamp int64, signature string, requestID uint64) []byte {
...
// buildAccountBalanceRequest builds a JSON message for v2/account.balance
func (c *BinanceFuturesExecutionClient) buildAccountBalanceRequest(timestamp int64, signature string, requestID uint64) []byte {
	var buf bytes.Buffer
	buf.Grow(256)

	buf.WriteString(`{"id":"`)
	buf.WriteString(strconv.FormatUint(requestID, 10))
	buf.WriteString(`","method":"v2/account.balance","params":{`)
...
func (c *BinanceFuturesExecutionClient) signEd25519(payload []byte) string {
	signature := ed25519.Sign(c.privateKey, payload)
	return base64.StdEncoding.EncodeToString(signature)
}

// buildOrderNewRequest builds a JSON message for order.place
func (c *BinanceFuturesExecutionClient) buildOrderNewRequest(symbol string, side common.Side, orderType common.OrderType, timeInForce common.TimeInForce, price float64, quantity float64, clientOrderID int, timestamp int64, signature string, requestID uint64) []byte {
	var buf bytes.Buffer
	buf.Grow(512)
...
func (c *BinanceFuturesExecutionClient) buildOrderCancelRequest(symbol string, orderID int, timestamp int64, signature string, requestID uint64) []byte {
	var buf bytes.Buffer
	buf.Grow(256)
...
func (c *BinanceFuturesExecutionClient) buildCancelAllOrdersRequest(symbol string, timestamp int64, signature string, requestID uint64) []byte {
	var buf bytes.Buffer
	buf.Grow(256)
...
func (c *BinanceFuturesExecutionClient) sendMessage(msg []byte) error {
	c.connLock.RLock()
	conn := c.conn
	c.connLock.RUnlock()
...
func (c *BinanceFuturesExecutionClient) pingLoop() {
...
func (c *BinanceFuturesExecutionClient) handleDisconnect() {
	c.connected.Store(false)
	...
	log().Warn().Int("accountID", c.accountID).Msg("Futures WebSocket API disconnected, attempting to reconnect...")
...
func (c *BinanceFuturesExecutionClient) processMessage(data []byte) {
	...
	result, dataType, _, err := jsonparser.Get(data, "result")
	if err == nil && len(result) > 0 {
		// v2/account.balance returns an array of asset balances
		if dataType == jsonparser.Array {
			c.processAccountBalanceResponse(result)
			return
		}
...
// Futures event types:
// - ACCOUNT_UPDATE: wallet/balance updates -> TopicEventBalanceUpdate (if subscribed)
// - ORDER_TRADE_UPDATE: order + fill updates (if subscribed)
// - Others -> logged and ignored
func (c *BinanceFuturesExecutionClient) processUserDataStreamEvent(data []byte) {
	...
	case "ORDER_TRADE_UPDATE":
		c.processOrderTradeUpdate(data)
	case "MARGIN_CALL", "ACCOUNT_CONFIG_UPDATE", "listenKeyExpired", "TRADE_LITE":
		log().Debug().
			Int("accountID", c.accountID).
			Str("eventType", eventType).
			Msg("Received futures user data event - not handled")
	default:
		log().Warn().
			Int("accountID", c.accountID).
			Str("eventType", eventType).
...
// processAccountUpdate processes ACCOUNT_UPDATE events and publishes BalanceUpdate.
// Format: {"e":"ACCOUNT_UPDATE","T":...,"a":{"B":[{"a":"USDT","wb":"...","cw":"..."}],"P":[...]}}
func (c *BinanceFuturesExecutionClient) processAccountUpdate(data []byte) {
	updateTime, _ := jsonparser.GetInt(data, "T")
	if updateTime == 0 {
		updateTime, _ = jsonparser.GetInt(data, "E")
	}

	balancesObj, _, _, err := jsonparser.Get(data, "a", "B")
	if err != nil || len(balancesObj) == 0 {
		return
	}

	balanceCount := 0
	_, _ = jsonparser.ArrayEach(balancesObj, func(value []byte, dataType jsonparser.ValueType, _ int, err error) {
		wbStr, _ := jsonparser.GetString(value, "wb")
		cwStr, _ := jsonparser.GetString(value, "cw")
		wb := parseFloat64([]byte(wbStr))
		cw := parseFloat64([]byte(cwStr))
		if wb > 0 || cw > 0 {
			balanceCount++
		}
	})
	...
	if balanceCount == 0 {
		log().Debug().Int("accountID", c.accountID).Msg("No non-zero balances in ACCOUNT_UPDATE")
		return
	}
	...
	balanceUpdate := event.BalanceUpdate{
		...
		WalletID:  c.walletID,
		Balances:  balances,
		UpdatedAt: uint64(updateTime) * 1_000_000,
	}
...
// processOrderTradeUpdate processes ORDER_TRADE_UPDATE events from the futures user data stream.
// Order fields live under "o".
func (c *BinanceFuturesExecutionClient) processOrderTradeUpdate(data []byte) {
	...
	if !subOrderUpdate && !subFill {
		log().Debug().
			Int("accountID", c.accountID).
			Msg("Received ORDER_TRADE_UPDATE but neither order update nor fill subscribed - ignoring")
		return
	}

	orderObj, _, _, err := jsonparser.Get(data, "o")
	if err != nil || len(orderObj) == 0 {
		return
	}

	orderID, _ := jsonparser.GetInt(orderObj, "i")
	clientOrderIDStr, _ := jsonparser.GetString(orderObj, "c")
	clientOrderID, _ := strconv.Atoi(clientOrderIDStr)
	status, _ := jsonparser.GetString(orderObj, "X")
	transactTime, _ := jsonparser.GetInt(orderObj, "T")
	if transactTime == 0 {
		transactTime, _ = jsonparser.GetInt(data, "T")
	}
	updatedAt := uint64(transactTime) * 1_000_000
	...
	case "PARTIALLY_FILLED":
		if subOrderUpdate {
			executedQtyStr, _ := jsonparser.GetString(orderObj, "z")
		...
		if subFill {
			c.publishFillFromOrderTradeUpdate(orderObj, clientOrderID, int(orderID), transactTime)
		}
	...
	case "REJECTED":
		if subOrderUpdate {
			c.publishOrderRejected(clientOrderID, int(orderID), "Order rejected", updatedAt)
		}
	...
	log().Debug().
		...
		Msg("Processed ORDER_TRADE_UPDATE")
}

// publishFillFromOrderTradeUpdate extracts fill data from the "o" object and publishes Fill.
func (c *BinanceFuturesExecutionClient) publishFillFromOrderTradeUpdate(orderObj []byte, clientOrderID int, orderID int, transactTime int64) {
	lastQtyStr, _ := jsonparser.GetString(orderObj, "l")
	...
	lastPriceStr, _ := jsonparser.GetString(orderObj, "L")
	lastPrice := parseFloat64([]byte(lastPriceStr))

	commissionStr, _ := jsonparser.GetString(orderObj, "n")
	commission := parseFloat64([]byte(commissionStr))

	commissionAsset, _ := jsonparser.GetString(orderObj, "N")
	tradeID, _ := jsonparser.GetInt(orderObj, "t")
	symbolStr, _ := jsonparser.GetString(orderObj, "s")
	sideStr, _ := jsonparser.GetString(orderObj, "S")
	symbolID := c.getFuturesSymbolID(symbolStr)
	side := parseSide(sideStr)
	isMaker, _ := jsonparser.GetBoolean(orderObj, "m")
	...
	log().Debug().
		...
		Msg("Published fill from ORDER_TRADE_UPDATE")
}

// publishOrderAccepted publishes an OrderAccepted event
func (c *BinanceFuturesExecutionClient) publishOrderAccepted(clientOrderID, orderID int, createdAt uint64) {
...
func (c *BinanceFuturesExecutionClient) publishOrderPartiallyFilled(clientOrderID, orderID int, executedQty float64, updatedAt uint64) {
...
func (c *BinanceFuturesExecutionClient) publishOrderFilled(clientOrderID, orderID int, executedQty float64, updatedAt uint64) {
	e := event.OrderFilled{
		ClientOrderID: clientOrderID,
		OrderID:       orderID,
		AccountID:     c.accountID,
...
func (c *BinanceFuturesExecutionClient) publishOrderCanceled(clientOrderID, orderID int, errorCode int, updatedAt uint64) {
...
func (c *BinanceFuturesExecutionClient) publishOrderRejected(clientOrderID, orderID int, reason string, updatedAt uint64) {
...
func (c *BinanceFuturesExecutionClient) publishOrderUnknownStatus(clientOrderID, orderID int, status string) {
...
// and executions arrive via ORDER_TRADE_UPDATE, so this method skips publishing them to
// avoid duplication. It only publishes as a fallback when the stream is not subscribed.
func (c *BinanceFuturesExecutionClient) processOrderResponse(data []byte) {
	if c.userDataStreamSubscribed.Load() {
		return
	}
...
func (c *BinanceFuturesExecutionClient) processFills(data []byte, clientOrderID int, orderID int) {
...
// processAccountBalanceResponse processes v2/account.balance result (JSON array) and
// publishes TopicEventRespBalanceSnapshot.
func (c *BinanceFuturesExecutionClient) processAccountBalanceResponse(data []byte) {
	balanceCount := 0
	_, _ = jsonparser.ArrayEach(data, func(value []byte, dataType jsonparser.ValueType, _ int, err error) {
		balStr, _ := jsonparser.GetString(value, "balance")
		availStr, _ := jsonparser.GetString(value, "availableBalance")
		bal := parseFloat64([]byte(balStr))
		avail := parseFloat64([]byte(availStr))
		if bal > 0 || avail > 0 {
			balanceCount++
		}
	})
	...
	if balanceCount == 0 {
		log().Debug().Msg("No non-zero balances found in account.balance response")
		return
	}
...
func (c *BinanceFuturesExecutionClient) getTokenID(asset string) int {
	if c.catalog != nil {
...
// getFuturesSymbolID resolves a venue symbol name to the perpetual instrument ID.
func (c *BinanceFuturesExecutionClient) getFuturesSymbolID(symbolName string) int {
	if c.catalog == nil || symbolName == "" {
		return 0
	}
	if sym, err := c.catalog.GetSymbolByUniversalTicker("BINANCE_PERPETUAL_" + symbolName); err == nil {
		return sym.ID
	}
	if sym, err := c.catalog.GetSymbolByExchangeAndName(c.account.Exchange, symbolName); err == nil {
		return sym.ID
	}
	return 0
}
...
func (h *wsExecEventHandler) OnOpen(socket *gws.Conn) {
	log().Info().Int("accountID", h.client.accountID).Msg("Binance Futures WebSocket API connection opened")
...
func (h *wsExecEventHandler) OnClose(socket *gws.Conn, err error) {
	log().Info().Err(err).Int("accountID", h.client.accountID).Msg("Binance Futures WebSocket API connection closed")
}
